...
class SistemaDB:
    ...
    def ventana_docentes(self):
        ...
        self.img_tk= self.cargar_imagen("sin_imagen.png", 50, 50)  
    ...
    def actualizar(self):
        ...
        self.nueva_img = self.cargar_imagen(ruta_imagen, 50, 50)

        self.imagen.config(image=self.nueva_img)
        self.imagen.image = self.nueva_img  # mantener referencia

    def cargar_imagen(self, ruta, ancho=50, alto=50):
        print("Carga imagen")
        try:
            # Intentar abrir la imagen
            self.img = Image.open(ruta)
            self.img = self.img.resize((ancho, alto))  # redimensionar
        except Exception:
            # Si falla, crear una imagen con cruz
            self.img = Image.new("RGB", (ancho, alto), "white")
